...
fn main() {
    ...
    let mut queries = [
        connection
            .prepare("SELECT id, ip_start, ip_end, country, stateprov, city FROM ips NOT INDEXED WHERE ip_start <= ?1 AND ip_end >= ?2")
            .unwrap(),
        connection
            .prepare("SELECT id, ip_start, ip_end, country, stateprov, city FROM ips WHERE ip_start <= ?1 AND ip_end >= ?2")
            .unwrap()
    ];
    ...
    loop {
        ...
        println!("Fetched in: {}ns", now.elapsed().as_nanos());
